...
import {
  ...
  Investment3DVisualization,
  PortfolioSphere
} from '../../components/AdvancedIllustrations';
...
export default function WealthGuidePage() {
  ...
  const retryLoad = async () => {
    ...
    setStaleData(false);
    try {
      const res = await axios.get('/api/active-funds', { timeout: 15000 });
      if (!res || res.status < 200 || res.status >= 300) throw new Error('Failed to load active funds');
      const json = res.data;
  ...
  const loadApiActive = async () => {
    ...
    setApiAError('');
    try {
      const res = await axios.get('/api/funds/active', { timeout: 15000 });
      if (!res || res.status < 200 || res.status >= 300) throw new Error('Failed to load active');
      const data = res.data;
  ...
  const loadApiTrending = async () => {
    ...
    setApiTError('');
    try {
      const res = await axios.get('/api/funds/trending', { timeout: 15000 });
      if (!res || res.status < 200 || res.status >= 300) throw new Error('Failed to load trending');
      const data = res.data;
  ...
  const addToWatchlist = async (w) => {
    try {
      const res = await axios.post('/api/watchlist', { schemeCode: w.schemeCode, schemeName: w.schemeName }, { headers: { 'Content-Type': 'application/json', 'x-user-id': 'demo-user' }, timeout: 15000 });
            ...
            <Grid item xs={12} md={6}>
              ...
              >
                <Box sx={{ position: 'relative' }}>
                  ...
                  >
                    Real-time interconnected fund ecosystem with dynamic data flow
                  </Typography>
                  <FinancialNetworkAnimation />
                </Box>
              </motion.div>
        ...
        <Box sx={{ mt: { xs: 8, md: 12 } }}>
          ...
          >
            <Grid container spacing={4} alignItems="center">
              <Grid item xs={12} md={6}>
                <Stack spacing={2}>
                  <span className="section-heading">Growth Analytics</span>
                  <Typography variant="h3" sx={{ fontWeight: 800, lineHeight: 1.2 }}>
                    Watch Your Wealth <span className="neon-teal">Multiply</span>
                  </Typography>
                  <Typography className="body-lg" sx={{ mb: 3 }}>
                    Our advanced analytics show the potential of systematic investing. 
                    See how consistent SIP investments can transform into substantial wealth over time.
                  </Typography>
                  
                  {/* Statistics */}
                  <Grid container spacing={3} sx={{ mt: 2 }}>
                    {[
                      { label: 'Average Returns', value: 12, suffix: '%' },
                      { label: 'Active Funds', value: 2500, suffix: '+' },
                      { label: 'Happy Investors', value: 45000, suffix: '+' }
                    ].map((stat) => (
        ...
        <Box sx={{ mt: { xs: 10, md: 14 } }}>
          ...
          >
            <Grid container spacing={6} alignItems="center">
              ...
              <Grid item xs={12} md={6}>
                <Stack spacing={3}>
                  <span className="section-heading">SIP Power</span>
                  <Typography variant="h3" sx={{ fontWeight: 800, lineHeight: 1.2 }}>
                    The Magic of <span className="neon-pink">Compounding</span>
                  </Typography>
                  <Typography className="body-lg">
                    Start with just ₹5,000 per month and watch it grow into a substantial corpus. 
                    The power of systematic investment and compounding can help you achieve your financial dreams.
                  </Typography>
                  ...
                  <Stack spacing={2}>
                    {[
                      { 
                        ...
                        desc: 'Regular investments build strong financial habits',
                        icon: '🎯'
                      },
                      { 
                        title: 'Rupee Cost Averaging', 
                        desc: 'Buy more units when prices are low, fewer when high',
                        icon: '⚖️'
                      },
                      { 
                        title: 'Long-term Wealth', 
                        desc: 'Time in the market beats timing the market',
                        icon: '📈'
                      }
                    ].map((benefit, i) => (
                      <motion.div
                      ...
                      >
                        ...
                        >
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            <Typography sx={{ fontSize: '2rem' }}>{benefit.icon}</Typography>
                            <Box>
        ...
        <Box sx={{ mt: { xs: 12, md: 16 } }}>
          ...
          >
            <Box sx={{ textAlign: 'center', mb: 8 }}>
              <span className="section-heading">Investment Journey</span>
              <Typography variant="h3" sx={{ fontWeight: 800, mt: 2, mb: 3 }}>
                Your Path to <span className="text-gradient">Financial Freedom</span>
              </Typography>
              <Typography className="body-lg" sx={{ maxWidth: 700, mx: 'auto' }}>
                Follow a proven investment strategy that has helped thousands of investors 
                build substantial wealth through disciplined mutual fund investing.
              </Typography>
            </Box>
            <InvestmentJourney />
          </motion.div>
