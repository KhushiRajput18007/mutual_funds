...
function FundsContent() {
  ...
  // Define filterSchemes before it is referenced in effects to avoid TDZ issues
  const filterSchemes = useCallback(() => {
    ...
    // Reset pagination to 8 when filters/search/sort change
    setItemsToShow(Math.min(filtered.length, 8));
  }, [searchTerm, schemes, sortBy, activeTab]);
  ...
  const fetchSchemes = async () => {
    // Try active funds first, but do not abort on error
    try {
      ...
      if (Array.isArray(dataActive) && dataActive.length > 0) {
        // Only consider active where nav > 0 as requested and normalize fields
        const activeByNav = dataActive
          .filter((s) => Number(s?.nav) > 0)
          .map((s) => ({
            ...s,
            schemeCode: s.schemeCode || String(s._id || ''),
            schemeName: s.schemeName || s.name || s?.meta?.scheme_name || 'Unnamed Fund'
          }));
        ...
        setItemsToShow(Math.min(activeByNav.length, 8));
        setLoading(false);
        return;
      }
    } catch (e) {
      // ignore and fall through to /api/mf
      console.warn('active-funds unavailable, falling back to /api/mf');
    }

    // Fallback to direct MF API
    try {
      ...
      if (Array.isArray(dataAll) && dataAll.length > 0) {
        ...
        // Show first 8 initially; user can load more
        setItemsToShow(Math.min(dataAll.length, 8));
      }
    } catch (e) {
      console.error('Error fetching /api/mf:', e);
    }
    setLoading(false);
  };
  ...
  const loadMore = () => {
    setItemsToShow(prev => Math.min(prev + 8, filteredSchemes.length));
  };
